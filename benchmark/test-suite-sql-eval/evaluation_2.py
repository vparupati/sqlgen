...
from exec_eval_2 import eval_exec_match


def ensure_timeout(seconds: float):
    ...
    def fn_wrapper(fn):
        ...
        def run(*args, **kwargs):
            ...
            def work(res):
                res['result'] = fn(*args, **kwargs)

            with Manager() as manager:
                res_dict = manager.dict()
                p = Process(target=work, args=(res_dict, ))
                p.start()
                p.join(seconds)
...
def evaluate(gold, predict, db_dir, db2_dir, etype, out_file, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            db2 = os.path.join(db2_dir, db2, db2 + ".sqlite")
            ...
            if etype in ["all", "exec"]:
                ...
                try:
                    exec_score = eval_exec_match(db=db, db2=db2, p_str=p_str, g_str=g_str, plug_value=plug_value,
            ...
            if etype in ["all", "match"]:
                ...
                p_valid_col_units = build_valid_col_units(p_sql['from']['table_units'], schema2)
    ...
    if out_file is not None:
        with open(out_file, 'w', encoding='utf-8') as f:
            json.dump(failed_pred, f, ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--db', dest='db', type=str, help="the directory that contains all the databases (for gold) and test suites")
    parser.add_argument('--db2', dest='db2', type=str, help="the directory that contains all the databases (for pred) and test suites")
    parser.add_argument('--out_file', dest='out_file', type=str, help="output file path")
    ...
    evaluate(args.gold, args.pred, args.db, args.db2, args.etype, args.out_file, kmaps, args.plug_value, args.keep_distinct, args.progress_bar_for_each_datapoint)
